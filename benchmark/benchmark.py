...
from implementation.cupy import kuwahara_cupy
...
def run_benchmarks() -> Dict:
    ...
    print(
        f"Benchmarking {len(IMPLEMENTATIONS)} implementation(s) on {len(sample_images)} sample(s)..."
    )
    ...
    for impl_name, impl_func in IMPLEMENTATIONS.items():
        ...
        for radius in RADII:
            ...
            print(
                f"    Average time for {impl_name} (radius {radius}): {radius_results['average_time']:.4f}s\n"
            )
...
def create_bar_chart(results: Dict):
    ...
    width = 0.8 / total_bars_per_sample
    ...
    ax.set_title(
        "Kuwahara Filter Implementation Benchmark", fontsize=14, fontweight="bold"
    )
    ax.set_xticks(x)
